...
- **WeasyPrint needs native libs and fonts not in the container** — `libgobject`, `libpango` etc. must be pre-downloaded to `~/deps/` and `LD_LIBRARY_PATH` set via `toolforge envvars`. On a fresh install: `cd ~/deps && apt-get download libglib2.0-0t64 libpango-1.0-0 libpangoft2-1.0-0 libfontconfig1 libfreetype6 libharfbuzz0b libffi8 libpcre2-8-0 libpixman-1-0 libpng16-16t64 libcairo2 libgraphite2-3 libexpat1 libbrotli1 libuuid1 && for deb in *.deb; do dpkg-deb -x "$deb" .; done && rm *.deb`, then `toolforge envvars create LD_LIBRARY_PATH /data/project/profile-creator-nlwiki/deps/usr/lib/x86_64-linux-gnu`. Also download `fontconfig fonts-dejavu-core fonts-liberation` and set both `toolforge envvars create FONTCONFIG_FILE /data/project/profile-creator-nlwiki/deps/etc/fonts/fonts.conf` and `toolforge envvars create LD_LIBRARY_PATH /data/project/profile-creator-nlwiki/deps/usr/lib/x86_64-linux-gnu`. `deploy.sh` regenerates `fonts.conf` on every deploy. Check missing libs with `scripts/debug/test_weasyprint.sh`.
